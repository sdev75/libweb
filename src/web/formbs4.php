...
class FormInput extends FormField{
	...
	public function __toString(){
		...
		if(!empty($this->data['desc'])){
			...
			$res .= "<small class=\"form-text text-muted\">{$desc}</small>";
		}
		return $res;
	}
...
class FormCheckbox2 extends FormField{
	...
	public function __toString(){
		...
		if($this->options){
			...
			foreach($this->options as $o){
				...
				$id = mt_rand();
				$res .= "<input class=\"form-check-input\" id=\"$id\" type=\"radio\" name=\"{$this['name']}\" value=\"{$value}\" {$attr}";
				$res .= "{$attr}{$disabled}{$required}{$checked}>";
				$res .= "<label class=\"form-check-label\" for=\"$id\">{$label}</label></div>";
			}
		}

		if(!empty($this['desc'])){
			$desc = htmlentities($this['desc'],ENT_QUOTES);
			$res .= "<small class=\"form-text text-muted\">{$desc}</small>";
		}
